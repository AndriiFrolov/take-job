...
public class JobMatcher {
    ...
    public boolean isJobMatches(Job job, Settings settings) throws Exception {
        logger.info("---------Checking if  job matches requirements " + job.getTitle() + " ----");
        boolean isDateMatches = isDateMatches(job.getDueDate(), settings.getDeadlineHoursFromNow());
        logger.info("------------{} Date {}", isDateMatches ? "PASSED" : "NOT MATCHES", job.getDueDate());

        boolean isPriceMatches = isPriceMatches(job.getPrice(), settings.getMinTotalPrice());
